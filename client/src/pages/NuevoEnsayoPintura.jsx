...
export default function NuevoEnsayoPintura() {
  ...
  const enviar = async (e) => {
    ...
    try {
      ...
      const ensayo = await api('/ensayos-pintura', {
        method: 'POST',
        body: { ...form, cliente_id: Number(form.cliente_id), ofs: todas }
      });
      navigate(`/pintura/${ensayo.id}`);
    } catch (e) {
      setError(e.message);
      setGuardando(false);
    }
  ...
  return (
    <form className="formulario" onSubmit={enviar}>
      <div className="encabezado-pagina">
        <h2>Nuevo ensayo de pintura</h2>
        <p className="descripcion">Se asigna el No. de ensayo al guardar; los ensayos y los espesores se agregan después, conforme se van terminando.</p>
      </div>
      ...
      <div className="tarjeta">
        ...
        <div className="fila">
          ...
          <label>Referencia
            <input value={form.referencia} onChange={setConAutocompletado('referencia')} required {...val('referencia')} list="pin-referencias" />
            <datalist id="pin-referencias">
              {piezasFiltradas.map(p => <option key={p.id} value={p.referencia}>{p.denominacion}</option>)}
            </datalist>
          </label>
          <label>Denominación
            <input value={form.denominacion} onChange={setConAutocompletado('denominacion')} required {...val('denominacion')} list="pin-denominaciones" />
            <datalist id="pin-denominaciones">
        ...
        <div className="fila">
          <label className="ancho-total">Información previa
            <input value={form.informacion_previa} onChange={set('informacion_previa')}
              placeholder="ej. piezas de validación de proceso de pintura" />
          </label>
        </div>
      </div>
      ...
      <div className="barra-guardar">
        ...
        <Link to="/pintura" className="boton secundario">Cancelar</Link>
      </div>
    </form>
  );
}
