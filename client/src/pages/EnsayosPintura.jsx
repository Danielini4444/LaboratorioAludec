...
export const folioPin = (folio) => `Pin_${String(folio).padStart(4, '0')}`;
...
export default function EnsayosPintura() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    let vigente = true;
    setCargando(true);
    api(`/ensayos-pintura?q=${encodeURIComponent(q)}&cliente_id=${clienteId}`)
    ...
    <div>
      <div className="encabezado-detalle">
        <div className="encabezado-pagina">
          <h2>Ensayos pintura</h2>
          <p className="descripcion">Informes de ensayos de piezas pintadas con No. de ensayo único (Pin_####)</p>
        </div>
        {esMetrologia(user) && <Link className="boton" to="/pintura/nuevo">+ Nuevo ensayo</Link>}
      </div>

      <AvisoReportesPendientes modulo="pintura" />
      ...
      {cargando ? (
            ...
            <tbody>
              {ensayos.map(e => (
                <tr key={e.id} className={e.anulado ? 'inactivo' : ''}>
                  <td><Link to={`/pintura/${e.id}`}>{folioPin(e.folio)}</Link></td>
        ...
        <div className="vacio-bloque">
          ...
          {filtrando
            ...
            : 'Todavía no hay ensayos de pintura creados.'}
