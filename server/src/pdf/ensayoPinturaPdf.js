// Informe de Ensayos de pintura (folio Pin_####): datos generales
...
// Característica, Observaciones, Conformidad), apartado de ESPESORES por pieza
// (puntos de medición con promedio, imagen y comentario), valoración final,
// apartado de fotos con descripción y firma digital con QR.
const fs = require('fs');
...
// el formato real del informe de pintura; actualizar aquí cuando exista.
...
module.exports = function generarEnsayoPinturaPdf(stream, ensayo, opciones = {}) {
  ...
  doc.fillColor('white').font('Helvetica-Bold')
    .fontSize(11).text('INFORME DE ENSAYOS DE PINTURA / PAINT TEST REPORT', bandaX + 6, MARGEN + 9, { width: ANCHO_UTIL - 150 - 115 - 12, align: 'center' });
  ...
  // ===== Espesores por pieza =====
  const piezas = ensayo.piezas || [];
  if (piezas.length) {
    salto(60);
    doc.rect(MARGEN, doc.y, ANCHO_UTIL, 16).fillAndStroke(PRIMARIO, PRIMARIO);
    doc.fillColor('white').font('Helvetica-Bold').fontSize(8.5)
      .text('ESPESORES / COATING THICKNESS', MARGEN + 6, doc.y + 4);
    doc.fillColor('black');
    doc.y += 10;
    ...
    for (const pieza of piezas) {
      ...
      doc.font('Helvetica-Bold').fontSize(9).fillColor(PRIMARIO)
        .text(`Pieza / Part ${pieza.numero}`, MARGEN, doc.y);
      doc.fillColor('black');
      doc.y += 4;
      ...
      // Imagen de la pieza (si existe y el archivo está en disco)
      if (pieza.tiene_imagen && pieza.imagen_archivo && fs.existsSync(path.join(UPLOADS, pieza.imagen_archivo))) {
        try {
          const info = doc.openImage(path.join(UPLOADS, pieza.imagen_archivo));
          const escala = Math.min(240 / info.width, 160 / info.height);
          const ancho = info.width * escala;
          const alto = info.height * escala;
          if (doc.y + alto + 12 > LIMITE_Y + 30) doc.addPage();
          doc.image(path.join(UPLOADS, pieza.imagen_archivo), MARGEN, doc.y + 2, { width: ancho, height: alto });
          doc.y += alto + 6;
          doc.x = MARGEN;
      ...
      if (pieza.comentario) {
        doc.font('Helvetica').fontSize(8.5).fillColor('black')
          .text(`Comentario / Comment: ${pieza.comentario}`, MARGEN, doc.y, { width: ANCHO_UTIL });
        doc.y += 4;
      }
      doc.y += 6;
    }
